                           ...
                           float accelX, float accelY, float accelZ)
{
    ...
    // Line 1: Temperature
    snprintf(line_buf, sizeof(line_buf), "T:%.1f C (obj) %.1f C", tempObj, tempAmb);
    printText(line_buf, 0, 0);

    // Line 2: HR and SpO2
    snprintf(line_buf, sizeof(line_buf), "HR:%d bpm  O2:%d%%", bpm, spo2);
    printText(line_buf, 0, 8);

    // Line 3: Acceleration
    snprintf(line_buf, sizeof(line_buf), "Ax:%.1f Ay:%.1f Az:%.1f", accelX, accelY, accelZ);
    printText(line_buf, 0, 16);
